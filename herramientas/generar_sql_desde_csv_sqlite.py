"""
Script para generar scripts SQL desde un CSV para cargar datos en SQLite (desarrollo local).
Ajusta los datos según los modelos y setea valores por defecto.

USO:
    python generar_sql_desde_csv_sqlite.py <archivo.csv> [stock_default]
    
    Ejemplo:
    python generar_sql_desde_csv_sqlite.py datos.csv 10
...
SALIDA:
    Genera un archivo .sql con el mismo nombre que el CSV, listo para ejecutar en SQLite.
    El script incluye:
    - INSERT de marcas (con INSERT OR IGNORE)
    - INSERT de categorías (con INSERT OR IGNORE)
    - INSERT de subcategorías (con INSERT OR IGNORE)
    - INSERT/UPDATE de productos (con INSERT OR REPLACE)
"""
import csv
import os
import sys
...
def convert_precio(precio):
    ...
    try:
        if isinstance(precio, str):
            # Limpiar formato de precio (quitar $, espacios, comas)
...
def generar_sql_marcas(marcas_unicas):
    """Genera SQL para insertar marcas (SQLite compatible)."""
    sql_lines = []
    sql_lines.append("-- ============================================")
    ...
    for idx, marca_nombre in enumerate(marcas_unicas, 1):
        ...
        sql_lines.append(f"INSERT OR IGNORE INTO productos_marca (nombre, descripcion, activo, fecha_creacion, fecha_actualizacion, fecha_eliminacion, eliminado_por_id)")
        sql_lines.append(f"VALUES ({escape_sql_string(nombre)}, {escape_sql_string(f'Marca {nombre}')}, 1, datetime('now'), datetime('now'), NULL, NULL);")
        sql_lines.append("")
...
def generar_sql_categorias(categorias_unicas):
    """Genera SQL para insertar categorías (SQLite compatible)."""
    sql_lines = []
    sql_lines.append("-- ============================================")
    ...
    for idx, categoria_nombre in enumerate(categorias_unicas, 1):
        ...
        sql_lines.append(f"INSERT OR IGNORE INTO productos_categoria (nombre, descripcion, activo, fecha_creacion, fecha_actualizacion, fecha_eliminacion, eliminado_por_id)")
        sql_lines.append(f"VALUES ({escape_sql_string(nombre)}, {escape_sql_string(f'Categoría {nombre}')}, 1, datetime('now'), datetime('now'), NULL, NULL);")
        sql_lines.append("")
...
def generar_sql_subcategorias(subcategorias_dict):
    """Genera SQL para insertar subcategorías (SQLite compatible)."""
    sql_lines = []
    sql_lines.append("-- ============================================")
    ...
    for (categoria_nombre, subcategoria_nombre) in subcategorias_dict:
        ...
        sql_lines.append(f"INSERT OR IGNORE INTO productos_subcategoria (categoria_id, nombre, descripcion, activo, fecha_creacion, fecha_actualizacion, fecha_eliminacion, eliminado_por_id)")
        sql_lines.append(f"SELECT id, {escape_sql_string(subcategoria)}, {escape_sql_string(f'Subcategoría {subcategoria}')}, 1, datetime('now'), datetime('now'), NULL, NULL")
        sql_lines.append(f"FROM productos_categoria WHERE nombre = {escape_sql_string(categoria)} LIMIT 1;")
        sql_lines.append("")
...
def generar_sql_productos(productos_data, stock_default=10):
    """Genera SQL para insertar productos (SQLite compatible)."""
    sql_lines = []
    sql_lines.append("-- ============================================")
    ...
    for producto in productos_data:
        ...
        unidad_raw = str(producto.get('unidaddetamano') or 'UD').upper().strip()
        ...
        sql_lines.append(f"INSERT OR REPLACE INTO productos_producto (")
        ...
        sql_lines.append(f"    datetime('now'),")
        sql_lines.append(f"    datetime('now'),")
        sql_lines.append(f"    NULL,")
        sql_lines.append(f"    NULL")
        sql_lines.append(f");")
...
def main():
    """Función principal."""
    if len(sys.argv) < 2:
        print("Uso: python generar_sql_desde_csv_sqlite.py <archivo.csv> [stock_default]")
        print("Ejemplo: python generar_sql_desde_csv_sqlite.py datos.csv 10")
        return
    ...
    print("\n" + "="*60)
    print("GENERANDO SCRIPTS SQL PARA SQLITE DESDE CSV")
    print("="*60)
    ...
    try:
        ...
        sql_output = []
        sql_output.append("-- ============================================")
        sql_output.append("-- SCRIPT SQL GENERADO DESDE CSV (SQLITE)")
        ...
        # Guardar archivo SQL
        sql_path = csv_path.replace('.csv', '_sqlite.sql')
        if sql_path == csv_path:
            sql_path = csv_path + '_sqlite.sql'
        ...
        print("\nPara cargar en SQLite:")
        print(f"  sqlite3 db.sqlite3 < {sql_path}")
        print("="*60 + "\n")
